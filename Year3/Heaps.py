...
def heapify(A, i, n=None):
    ...
    if (lefty < n and A[lefty] < A[smallest]):
        smallest = lefty
    if (righty < n and A[righty] < A[smallest]):
        smallest = righty
    if (smallest != i):
        swap(A, i, smallest)
...
def buildHeap(A):
    ...
    n = None
    start = int(len(A)/2 - 1) # starting at the half way pt (minus 1) call heapify
...
def heapInsert(A, v):
    ...
    A.append(v)
    i = len(A)-1
    while(parent(i) > -1 and A[parent(i)] > A[i]):
        swap(A, parent(i), i)
        i = parent(i)
...
def main():
    ...
    print("Heap size 20:")
    printCompleteTree(A)
    print("adding value 8 to the tree")
    heapInsert(A, 8)
    printCompleteTree(A)
